...
@if (session('csrf_error')) {{ session('csrf_error') }} @endif
    ...
    @php Former::populate($role); @endphp
    ...
    <div class="m-portlet__body">
        <div class="m-form__section m-form__section--first">
            ...
            <div class="form-group m-form__group row m--margin-top-10">
                ...
                <div class="col-4">
                    <input class="form-control m-input" type="text" value="{{$role->name}}" placeholder="" id="title" name="role_name">
                </div>
            </div>
                ...
                <div class="col-4">
                    <select class="form-control m-bootstrap-select m_selectpicker" name="status" id="status">
                        ...
                        @foreach ($status_array as $key => $value)
                            @if(isset($role))
                                <option value="{{$key}}" {{($role->status == $key)?('selected'):('')}}>@lang('admin.'.$value.'')</option>
                            @else
