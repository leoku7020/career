<?php
namespace App\Utils\Former\Framework;
...
/**
 * Metronic 5.2 form framework
 */
class Metronic5 extends Framework implements FrameworkInterface
{
	/**
	...
	{
		return 'form-group m-form__group';
	}
	/**
	 * Add label classes
	 *
     ...
     * @return Element
     */
    public function createError($field, $errors)
    {
        if ($errors->has($field)) {
        return Element::create('div')->addClass('form-control-feedback')
            ->nest('strong', $errors->first($field));
        }
    }
	...
	{
		return Element::create('span', $text, $attributes)->addClass('m-form__help');
	}
	/**
	 * Render an help text
	 *
	...
	{
		return Element::create('p', $text, $attributes)->addClass('m-form__help');
	}
	/**
