    ...
    */
    var initFileInput = function() {
        ...
        $('.multiple').on('change',function(){
            ...
            var limit_ext = Array('jpg','png','doc','docx','xls','xlsx','pdf','csv'); //限制格式
            var Msg = '';
            $.each(files, function( index, value ) {
                if($.inArray(value.name.replace(/^.*\./, '').toLowerCase(),limit_ext) == "-1"){
                    Msg += "格式不符:"+value.name+'<br>';
                }
                if(value.size > 5242880){
                    Msg += "檔案大小不符:"+value.name+'<br>';
                }
            });
            if(Msg == ''){
                var Name = '已選擇'+$(this)[0].files.length+'個檔案';
                $(this).next('.custom-file-label').addClass("selected").html(Name);
            }else{
                swal("錯誤", Msg, "error");
                $(this).next('.custom-file-label').val('');
                $(this).next('.custom-file-label').html('請選擇檔案');
            }
